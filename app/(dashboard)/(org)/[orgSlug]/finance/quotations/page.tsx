...
import {
  ...
  ArrowLeftFromLine, Trash2, Loader2, FileInput, Download
} from 'lucide-react'
...
import {
  ...
  duplicateQuotation, convertQuoteToInvoice, convertEstimateToQuote
...
export default function QuotationsPage({ params }: { params: Promise<{ orgSlug: string }> }) {
  ...
  const filtered = quotations.filter(q => {
    ...
    return (
      (q.quotation_number || '').toLowerCase().includes(ql) ||
