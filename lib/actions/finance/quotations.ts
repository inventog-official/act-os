...
import { generateNumber, calculateDocumentTotals, calculateItemTax, getTaxRatesMap, getCurrentUser } from './utils'
import { sendEmail, buildQuoteEmailHtml, getOrganizationEmailSettings } from '@/lib/email'
...
export async function updateQuotation(id: string, input: {
  ...
  if (input.items) {
    ...
    const existing = await db.select({ organizationId: financeQuotations.organizationId })
      .from(financeQuotations)
...
export async function sendQuotationEmail(id: string) {
  ...
  try {
    if (!q.client_email) {
      ...
      const org = await getOrganizationEmailSettings(q.organization_id)
      const doc: PdfDocument = {
        title: q.type === 'estimate' ? 'Estimate' : 'Quote',
        ...
        discountAmount: String(q.discount_amount || '0'),
        taxAmount: String(q.tax_amount || '0'),
        total: String(q.total),
        notes: q.notes,
        terms: q.terms,
        status: q.status,
        orgName: org?.name || 'Quote',
      }
      const pdf = await generatePdf(doc)
      const result = await sendEmail({
        to: q.client_email,
        subject: `${q.type === 'estimate' ? 'Estimate' : 'Quote'} ${q.quote_number} from ${org?.name || 'us'}`,
        html: buildQuoteEmailHtml({
          ...
          status: 'sent',
          expiryDate: q.expiry_date ? new Date(q.expiry_date).toLocaleDateString() : null,
          notes: q.notes,
        }),
        attachments: [{ filename: `Quote_${q.quote_number}.pdf`, content: Buffer.from(pdf), contentType: 'application/pdf' }],
      })
