...
export async function getRevenueByProject(organizationId: string) {
  ...
  const ids = Object.keys(grouped).filter(id => id !== 'unassigned')
  ...
  if (ids.length) {
    const { data: projects } = await supabase
      ...
      .in('id', ids)
      .is('deleted_at', null)
    names = Object.fromEntries((projects || []).map((p: any) => [p.id, p.name]))
  }

  return Object.entries(grouped).map(([projectId, total]) => ({
    projectId,
    projectName: projectId === 'unassigned' ? 'Unassigned' : names[projectId] || 'Unknown project',
    total,
  }))
}
