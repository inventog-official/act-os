...
ALTER TABLE finance_products
  ADD CONSTRAINT finance_products_tax_rate_id_fkey
  FOREIGN KEY (tax_rate_id) REFERENCES finance_tax_rates(id) ON DELETE SET NULL;
...
CREATE INDEX idx_finance_product_categories_org ON finance_product_categories(organization_id, deleted_at);
CREATE INDEX idx_finance_tax_rates_org ON finance_tax_rates(organization_id, deleted_at);
CREATE INDEX idx_finance_expense_categories_org ON finance_expense_categories(organization_id, deleted_at);
CREATE INDEX idx_finance_quotation_items_quote ON finance_quotation_items(quotation_id);
CREATE INDEX idx_finance_invoice_items_invoice ON finance_invoice_items(invoice_id);
CREATE INDEX idx_finance_purchase_order_items_po ON finance_purchase_order_items(purchase_order_id);
CREATE INDEX idx_finance_purchase_orders_org_status ON finance_purchase_orders(organization_id, status);
...
CREATE POLICY "Members can view product categories" ON finance_product_categories FOR SELECT
