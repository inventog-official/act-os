-- ACT OS Phase 7 — Completion fixes (2)
-- Fix adjust_stock_atomic so 'transfer' movements are signed (bidirectional):
--   createTransfer  passes -quantity  -> deduct from source warehouse
--   receiveTransfer passes +quantity  -> add to destination warehouse
-- Previously 'transfer' always deducted, so receiveTransfer removed stock from
-- the destination instead of adding it.
...
BEGIN
  ...
  IF p_movement_type IN ('receipt', 'opening_balance', 'production') THEN
    v_delta := v_abs;
  ELSIF p_movement_type IN ('issue', 'return', 'sale', 'allocation', 'consumption') THEN
    v_delta := -v_abs;
  ELSIF p_movement_type IN ('adjustment', 'correction', 'transfer') THEN
  ...
  IF p_movement_type IN ('adjustment', 'correction', 'transfer') THEN
